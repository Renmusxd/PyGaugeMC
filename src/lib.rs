...
impl GaugeTheory {
    ...
    #[new]
    fn new(
        t: usize, x: usize, y: usize, z: usize, vs: Vec<f64>,
        seed: Option<u64>
    ) -> Self {
    ...
    fn add_flux(&mut self, t: usize, x: usize, y: usize, z: usize, p: usize, amount: Option<i32>) {
        self.graph.add_flux(SiteIndex { t,x,y,z }, p, amount.unwrap_or(1));
    }

    fn update_all_cubes(&mut self, dima: usize, dimb: usize, dimc: usize, offset: Option<bool>) {
        let dims = [Dimension::from(dima), Dimension::from(dimb), Dimension::from(dimc)];
        ...
        NDDualGraph::apply_cube_updates(&cube_choices, &dims, leftover, off, self.graph.graph_state_mut(), &bounds);
    }
